...
gendoc: $(DOCDIR)
	cp $(SRC)/docs/* $(DOCDIR) ; \
	$(RUN) gen-doc --include-top-level-diagram --diagram-type er_diagram  ${GEN_DARGS} -d $(DOCDIR) $(SOURCE_SCHEMA_PATH)
...
git-init:
	git init
git-add: .cruft.json
	git add .gitignore .github .cruft.json Makefile LICENSE *.md examples utils about.yaml mkdocs.yml poetry.lock project.Makefile pyproject.toml src/mtg_ontology/schema/*yaml src/*/datamodel/*py src/data src/docs tests src/*/_version.py
